...
Fetches recent user feedback for the [Omi](https://www.omi.me) **mobile app**
— App Store reviews and Google Play reviews, no GitHub — groups it into
themes, scores each theme with the Priority Score formula from Omi's own
...
Scoped to the mobile app on purpose: GitHub's issue tracker covers every Omi
surface (macOS, backend, firmware) and mixes real user bugs with internal
engineering/CI/process tickets no end user ever sees. Recent App Store and
Play Store reviews are direct, unfiltered mobile-app end-user signal —
that's the only thing this report ranks.
...
optional: `USER_SIGNALS_APPSTORE_ID` (default `6502156163`),
`USER_SIGNALS_PLAYSTORE_ID` (default `com.friend.ios`),
`USER_SIGNALS_LOOKBACK_DAYS`, `USER_SIGNALS_REVIEW_CAP`,
`USER_SIGNALS_TOP_N`, `USER_SIGNALS_OUT_DIR` — see `user_signals/config.py`.
Point it at a different app by overriding the two IDs.

## How it works

```
sources_appstore.py  ─┐
                       ├─► Signal (models.py) ─► classify.py ─► score.py ─► report.py
...
- **Sources** (`user_signals/sources_*.py`) each expose `fetch(...) -> list[Signal]`
  ...
  auth needed for either: App Store is a public RSS feed, Play Store uses
  `google-play-scraper`, which reads the same public data the Play Store
  app itself shows.
...
- **score.py** implements `ISSUE_TRIAGE_GUIDE.MD` section 5's formula —
  ...
  `Maintenance Leverage` rewards cross-*store* corroboration: a theme both
  App Store and Play Store users raise independently outranks a one-store
  complaint.
...
- Apple's review RSS feed caps at ~500 reviews per storefront; Play Store
  pulls are capped by `USER_SIGNALS_REVIEW_CAP` — this is a snapshot of
  recent signal, not a full census.
- Out of scope by design, not by omission: GitHub issues, macOS/desktop
  bugs, backend/firmware tickets. If you want that signal too, run
  [`ISSUE_TRIAGE_GUIDE.MD`](./ISSUE_TRIAGE_GUIDE.MD)-style triage on the
  repo directly rather than folding it back into this pipeline.
...
— keep any new source scoped to end-user mobile-app feedback to match the
rest of this pipeline.

**Discord is a planned 3rd source, not yet built** — needs a bot token with

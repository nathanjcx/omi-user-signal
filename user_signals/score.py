...
GitHub's issue tracker mixes real user bugs with internal engineering/
CI/process tickets and covers every Omi surface (macOS, backend, firmware),
not just the mobile app, so it's out of scope here by design, not omission.
"""
...
def _frequency(signals: list[Signal], lookback_days: int) -> int:
    ...
    one viral review doesn't read as "happens daily" on its own."""
...
def _maintenance_leverage(signals: list[Signal]) -> int:
    """Cross-store corroboration is the leverage signal available
    automatically: a theme both App Store and Play Store users raise is
    more likely a systemic fix than one store's one-off complaint."""
